...
save_dir = fullfile(project_root, 'LS2026\identification', 'figs', 'open_loop_identification');
...
x_limits = [t(1) t(2750)];   % alebo [min(t) max(t)]
...
grid on; box on;
ylabel('Pump F speed (\%)','Interpreter','latex');
title('Input signal','Interpreter','latex');
legend('Training data','Test data','Location','southwest','Interpreter','latex');
ylim([0 101]);
xlim(x_limits);
hold off;
...
title('Measured output','Interpreter','latex');
legend('Training data','Test data','Location','best','Interpreter','latex');
xlim(x_limits);
hold off;
...
fig2 = figure('Color','w','Name','Strejc Open Loop');
subplot(2,1,1)
...
% plot
fig3 = figure('Color','w','Name','Koopman Open Loop');
subplot(2,1,1)
...
%% plot compare
fig4 = figure('Color','w','Name','Open Loop Model Comparison');
...
legend('True output','Linear model','Koopman model','Location','best');

%% ===== SAVE FIGURES =====
savefig(fig1, fullfile(save_dir, 'raw_identification_measurements.fig'));
exportgraphics(fig1, fullfile(save_dir, 'raw_identification_measurements.png'), 'Resolution', 300);

savefig(fig2, fullfile(save_dir, 'strejc_open_loop.fig'));
exportgraphics(fig2, fullfile(save_dir, 'strejc_open_loop.png'), 'Resolution', 300);

savefig(fig3, fullfile(save_dir, 'koopman_open_loop.fig'));
exportgraphics(fig3, fullfile(save_dir, 'koopman_open_loop.png'), 'Resolution', 300);

savefig(fig4, fullfile(save_dir, 'open_loop_model_comparison.fig'));
exportgraphics(fig4, fullfile(save_dir, 'open_loop_model_comparison.png'), 'Resolution', 300);
