...
%% ================= SCALING =================
load('train_data.mat'); load('test_data.mat');

Yall = [Ytrain(:); Ytest(:)];
Uall = [Utrain(:); Utest(:)];

x_mean = mean(Yall);   x_std = std(Yall);
u_mean = mean(Uall);   u_std = std(Uall);
...
% Scaled bounds (used inside MPC)
umin = (20 - u_mean)/u_std;
umax = (100 - u_mean)/u_std;
ymin = (0 - x_mean)/x_std;
ymax = (70 - x_mean)/x_std;

% ---- COST SCALING FOR PHYSICAL DOMAIN ----
Q_real = Qy/(x_std^2);
R_real = Qu/(u_std^2);
...
x = sdpvar(repmat(nxk,1,N+1),repmat(1,1,N+1));
x0 = sdpvar(nxk,1);

con = [x{1}==x0];  obj = 0;
for k=1:N
    con = [con, x{k+1}==A_k*x{k}+B_k*u{k}];
    yk = C_k*x{k};
    con = [con, umin<=u{k}<=umax, ymin<=yk<=ymax];
    obj = obj + Qy*yk^2 + Qu*u{k}^2;
end
koopman_ctrl = optimizer(con,obj,sdpsettings('solver','quadprog'),x0,u{1});
...
con = [x{1}==x0]; obj = 0;
for k=1:N
    con = [con, x{k+1}==A_s*x{k}+B_s*u{k}];
    yk = C_s*x{k};
    con = [con, umin<=u{k}<=umax, ymin<=yk<=ymax];
    obj = obj + Qy*yk^2 + Qu*u{k}^2;
end
strejc_ctrl = optimizer(con,obj,sdpsettings('solver','quadprog'),x0,u{1});

%% ================= INITIAL CONDITIONS =================
y0_vals = [45, 50, 55, 58, 60, 62, 66, 68];
y0_scaled = (y0_vals - x_mean) / x_std;
...
%% ================= SIMULATION LOOP =================
for i=1:length(y0_scaled)

    %% ---- KOOPMAN ----
    xk = pinv(C_k)*y0_scaled(i);
    xest = xk;   P = eye(nxk);

    ytrue = zeros(1,sim_length+1);
    ucl   = zeros(1,sim_length);

    ytrue(1)=y0_scaled(i);
    py.baseline_inference.get_x(ytrue(1));

    for t=1:sim_length
        ucl(t) = koopman_ctrl{xest};
        ytrue(t+1) = double(py.baseline_inference.y_plus(ucl(t)).item());

        % KF
        xpred = A_k*xest+B_k*ucl(t);
        Ppred = A_k*P*A_k'+0.5*eye(nxk);
        K = (Ppred*C_k')/(C_k*Ppred*C_k'+0.1);
        xest = xpred + K*(ytrue(t+1)-C_k*xpred);
        P = (eye(nxk)-K*C_k)*Ppred;
    end

    yk = ytrue*x_std+x_mean;
    uk = ucl*u_std+u_mean;
    e  = yk(1:end-1)-x_mean;

    rmse = sqrt(mean(e.^2));
    iae  = sum(abs(e));
    obj  = sum(Q_real*e.^2 + R_real*(uk-u_mean).^2);

    results_koop = [results_koop; rmse iae obj];

    %% ---- STREJC ----
    xs = y0_scaled(i);  xest = xs;  P=1;
    ytrue = zeros(1,sim_length+1);
    ucl   = zeros(1,sim_length);
    ...
    for t=1:sim_length
        ucl(t) = strejc_ctrl{xest};
        ytrue(t+1) = double(py.baseline_inference.y_plus(ucl(t)).item());

        % KF
        xpred = A_s*xest+B_s*ucl(t);
        Ppred = A_s*P*A_s'+0.5;
        K = Ppred/(Ppred+0.1);
        xest = xpred + K*(ytrue(t+1)-xpred);
        P = (1-K)*Ppred;
    end

    ys = ytrue*x_std+x_mean;
    us = ucl*u_std+u_mean;
    e  = ys(1:end-1)-x_mean;

    rmse = sqrt(mean(e.^2));
    iae  = sum(abs(e));
    obj  = sum(Q_real*e.^2 + R_real*(us-u_mean).^2);

    results_str = [results_str; rmse iae obj];
end

fprintf('\n===== PERFORMANCE METRICS PER INITIAL TEMPERATURE =====\n');

for i = 1:length(y0_vals)
    fprintf('\nInitial temperature T0 = %.0f °C\n', y0_vals(i));
    
    fprintf('  Koopman MPC : RMSE = %.3f °C,  IAE = %.1f,  Obj = %.2f\n', ...
        results_koop(i,1), results_koop(i,2), results_koop(i,3));
    
    fprintf('  Strejc  MPC : RMSE = %.3f °C,  IAE = %.1f,  Obj = %.2f\n', ...
        results_str(i,1), results_str(i,2), results_str(i,3));
end

fprintf('\n===== AVERAGE PERFORMANCE OVER ALL INITIAL CONDITIONS =====\n');

mean_koop = mean(results_koop, 1);
mean_str  = mean(results_str, 1);

fprintf('Koopman MPC : RMSE = %.3f °C,  IAE = %.1f,  Obj = %.2f\n', ...
    mean_koop(1), mean_koop(2), mean_koop(3));

fprintf('Strejc  MPC : RMSE = %.3f °C,  IAE = %.1f,  Obj = %.2f\n', ...
    mean_str(1), mean_str(2), mean_str(3));
