...
K = mean(avg_step(end-6:end), 'omitnan'); %1.1691
target_value = 0.632 * K;% 68
...
% Annotate plot
hold on;
yline(target_value, '--', '$63.2\%$ of $\mathrm{K}$', 'Interpreter', 'latex');
if ~isempty(tau_idx)
    xline(tau_idx, '--r', 'T', 'Interpreter', 'latex','LabelVerticalAlignment', 'bottom');
end
legend('Average Response',  '$63.2\%$ of $\mathrm{K}$', '$\mathrm{T}$','Location','best', 'Interpreter', 'latex');
hold off;
