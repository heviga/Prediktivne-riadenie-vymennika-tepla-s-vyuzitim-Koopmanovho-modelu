...
c_u = 0.02;          % "price" of actuator use
c_y = 0.1;           % economic penalty for deviation from preferred y
...
y_pref = (55 - x_mean) / x_std;   % economically preferred output (e.g. 55 °C)
...
for k = 1:N
    ...
    objective = objective + c_u*(u{k}^2) + c_y*((yk - y_pref)^2);
...
for t = 1:sim_length
    ...
    y_baseline_array = double(py.array.array('d', py.numpy.nditer(y_baseline)));
...
econ_cost = sum(c_u*(u_cl_desc.^2) + c_y*((y_true_desc(1:end-1) - y_pref).^2));
fprintf('Celkové ekonomické náklady = %.4f\n', econ_cost);
...
save('results_koopman.mat', 'y_true_desc', 'y_est_desc', 'u_cl_desc','x_mean','u_mean','u_cl',"y_true");   % From Koopman
